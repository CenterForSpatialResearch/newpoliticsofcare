...
//var allData = d3.csv("County_level_coverage_for_all_policies_and_different_base_case_capacity_withCurrent.csv")
...
function ready(counties,outline,centroids,modelData,timeStamp,states){
   ...
   loader()
    ...
    map.once("idle",function(){
        colorWithCoverage(map)
        colorWithoutCoverage(map)
    })
    ...
    for(var i in data){
        ...
        for(var j in measureSet){
           ...
           // console.log(data[i][priorityKey])
            ...
            // var priorityGroupKey = priorityKey+"_group"
//             values[priorityGroupKey]=pGroup
            
            for(var k in coverageSet){
                var coverage = parseFloat(100-parseFloat(data[i][coverageKey]))
                if(coverage==-1){
                    var cGroup = "none"
                }else{
              ...
              // console.log(coverageKey)
                var combinedGroup = (parseInt(parseInt(pGroup)*3)+parseInt(parseInt(cGroup)+1))
                var combinedHeader = coverageKey+"_group"
              //  break
              
                //console.log([cGroup,pGroup,combinedGroup,combinedHeader])
                values[combinedHeader]="_"+combinedGroup
         ...
         if(feature["properties"].FIPS!=undefined){
           ...
           //  var columnsToShow = ["hotspotSVI_priority","hotspot_priority","SVI_priority","highDemand_priority"]       
            ...
            var currentSelection = pub.strategy+"_"+pub.coverage
             ...
             var groupLabels = {
                 _1:"Low Priority Score, Low % needs unmet",
                 _2:"Low Priority Score, Medium % needs unmet",
                 _3:"Low Priority Score, High % needs unmet",
                 _4:"Medium Priority Score, Low % needs unmet",
                 _5:"Medium Priority Score, Medium % needs unmet",
    ...
    var gridSize = 70
     ...
     for (var i = 0; i < measureSet.length; i++) {
         ...
         if(measureSet[i]!= "percentage_scenario_SVI_pop"){
             svg.append("rect").attr("width",gridSize-4).attr("height",gridSize-4)
             .attr("x",(i-1)%2*gridSize)
             .attr("y",Math.floor((i-1)/2)*gridSize)
             .attr("id",id)
             ...
             .on("click",function(){
                         ...
                         d3.selectAll("."+clickedId+"_radialS").style("background-color",highlightColor)
            ...
            //  .attr("x",(i-1)%2*gridSize+gridSize/2)
            //  .attr("y",Math.floor((i-1)/2)*gridSize+gridSize/2)
...
//          var label = row.append("div").html(displayText).attr("class",id+"_labelS labelS"+id)
...
function colorWithCoverage(map){
